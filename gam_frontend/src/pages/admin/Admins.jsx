  ...
  return (
    <div className="admins-page">
      <div className="page-header" style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: 'var(--br-text)' }}>Admin Control Panel</h1>
          <p style={{ fontSize: '13px', color: 'var(--color-text-subtle)' }}>Manage system administrators, custom roles, and fine-grained permissions.</p>
        </div>
        <div>
          {activeTab === 'admins' ? (
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                              ...
                              <div>
                                <div style={{ fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px' }}>
                                  {admin.name}
                                  {isPrimary && <span className="badge badge-warning" style={{ fontSize: '10px' }}>Primary</span>}
                                  {isSelf && <span className="badge badge-info" style={{ fontSize: '10px' }}>You</span>}
                                </div>
                              </div>
                            </div>
                          </td>
                          ...
                          <td>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                              {admin.roles && admin.roles.length > 0 ? (
                                ...
                                <span style={{ color: 'var(--color-text-subtle)', fontSize: '12px' }}>No Role</span>
                              )}
                              {admin.permissions && admin.permissions.length > 0 && (
                                <span className="override-pill" title={`${admin.permissions.length} direct overrides`}>
                                  +{admin.permissions.length} overrides
                                </span>
                              )}
                            </div>
                          </td>
                  ...
                  <div key={role.id} className="role-card card">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                      <div>
                        ...
                        {isSystem && <span style={{ fontSize: '10px', color: 'var(--color-text-subtle)', fontWeight: 'bold', textTransform: 'uppercase' }}>System Role</span>}
                      </div>
                    ...
                    <div style={{ flex: 1, minHeight: '80px', marginBottom: '12px' }}>
                      <p style={{ fontSize: '12px', color: 'var(--color-text-subtle)', marginBottom: '8px', fontWeight: 600 }}>Granted Permissions ({role.permissions?.length || 0}):</p>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                        {role.permissions && role.permissions.length > 0 ? (
                          ...
                          <span style={{ color: 'var(--color-text-subtle)', fontSize: '11px', fontStyle: 'italic' }}>No permissions granted.</span>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            <form onSubmit={handleAdminSubmit}>
              <div className="modal-body" style={{ maxHeight: 'calc(100vh - 250px)', overflowY: 'auto', paddingRight: '8px' }}>
                ...
                {editingAdmin && primaryAdmin && editingAdmin.id === primaryAdmin.id && (
                  <div style={{ display: 'flex', gap: '10px', background: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.2)', padding: '12px', borderRadius: '8px', marginBottom: '16px' }}>
                    ...
                    <p style={{ fontSize: '12px', color: '#f59e0b', margin: 0 }}>
                      <strong>Primary Administrator Lock:</strong> This account is the platform setup account. Deactivation is disabled, and it will always retain the <strong>Super Admin</strong> role with all permissions.
                    </p>
                  </div>
                )}
                ...
                <div className="form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
                  ...
                  <div className="form-group">
                    <label className="form-label">{editingAdmin ? 'Change Password (Optional)' : 'Password'}</label>
                    <div className="input-with-icon">
                      <Lock size={16} className="input-icon" />
                      <input 
                        type="password" 
                        className="form-input" 
                        placeholder={editingAdmin ? 'Leave blank to keep current' : 'Enter 8+ characters'}
                  ...
                  <div className="form-group" style={{ display: 'flex', alignItems: 'center', marginTop: '24px' }}>
                    <label className="checkbox-label" style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontSize: '13px' }}>
                      ...
                      />
                      <span>Active Account (allow login)</span>
                    </label>
                  </div>
                </div>
                ...
                <div style={{ marginBottom: '20px' }}>
                  <h4 style={{ fontSize: '13px', fontWeight: 'bold', marginBottom: '8px', color: 'var(--br-text)' }}>Assign Roles</h4>
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                    <h4 style={{ fontSize: '13px', fontWeight: 'bold', color: 'var(--br-text)', margin: 0 }}>Direct Permission Overrides</h4>
                    <span style={{ fontSize: '11px', color: 'var(--color-text-subtle)' }}>💡 Inherited permissions are checked & disabled</span>
                  </div>
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                                    ...
                                    {isSuperAdminUser && <span style={{ fontSize: '9px', color: 'var(--color-primary-light)' }}>Granted via Super Admin</span>}
                                    {!isSuperAdminUser && isInherited && <span style={{ fontSize: '9px', color: 'var(--color-warning)' }}>Inherited from role</span>}
                                    {!isSuperAdminUser && !isInherited && isDirect && <span style={{ fontSize: '9px', color: 'var(--color-success)' }}>Direct override</span>}
                                  </div>
                                </label>
                              )
                            })}
      ...
      {roleModalOpen && (
        <div className="modal-backdrop">
          <div className="modal role-modal" style={{ maxWidth: '600px', width: '90%' }}>
            <div className="modal-header">
              <h3>{editingRole ? 'Edit Role Permissions' : 'Create Custom Role'}</h3>
                ...
                <div className="form-group" style={{ marginBottom: '16px' }}>
                  ...
                  {editingRole && systemRoles.includes(editingRole.name) && (
                    <span style={{ fontSize: '11px', color: 'var(--color-text-subtle)', marginTop: '4px', display: 'block' }}>
                      ⚠️ System role names cannot be modified.
                    </span>
                  )}
                </div>

                <div style={{ marginBottom: '12px' }}>
                  <h4 style={{ fontSize: '13px', fontWeight: 'bold', color: 'var(--br-text)', marginBottom: '8px' }}>Granted Permissions</h4>

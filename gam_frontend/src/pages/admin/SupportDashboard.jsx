...
import {
  Ticket, Users, Clock, AlertCircle, RefreshCw, Megaphone, Search, ArrowRight, CheckCircle
} from 'lucide-react'
...
export default function SupportDashboard() {
  ...
  async function loadData() {
    ...
    try {
      ...
      setTickets(ticketsList.filter(t => t.status !== 'resolved' && t.status !== 'closed').slice(0, 6))
    ...
    <div>
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle" style={{ color: 'var(--color-text-muted)' }}>
            Workspace for managing ticket lifecycles, publisher verification, and broadcast announcements
          </p>
        </div>
      </div>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: .5, color: 'var(--color-text-muted)', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
          <Ticket size={14} style={{ color: 'var(--br-primary)' }} /> Ticket Queue & Users
        </div>
        <div className="stat-grid">
          ...
          <div className="stat-card warning" style={{ background: 'linear-gradient(135deg, rgba(245,158,11,0.1), rgba(245,158,11,0.02))', border: '1px solid rgba(245,158,11,0.3)' }}>
            ...
            <div className="stat-change text-muted">Awaiting activation review</div>
          </div>
        </div>
      </div>

      {/* Main Grid: Active Tickets & Search / Announcements */}
          ...
          <div className="card-header" style={{ padding: '16px 20px' }}>
            <div>
              <div className="card-title" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <Ticket size={16} style={{ color: 'var(--br-primary)' }} /> Support Queue
              </div>
              <div className="card-subtitle">Active tickets requiring attention</div>
            </div>
          </div>
              ...
              <tbody>
                {tickets.length === 0 ? (
                  <tr>
                    <td colSpan={5}>
                      <div style={{ textAlign: 'center', padding: '30px 10px', color: 'var(--color-text-muted)' }}>
                        All clear! No pending support tickets.
                      </div>
                    </td>
                  </tr>
                ) : (
                  tickets.map(t => (
                    <tr key={t.id}>
                      <td>
                        <div style={{ fontWeight: 600, fontSize: 13 }}>{t.publisher?.name}</div>
                        <div style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>{t.publisher?.email}</div>
                      </td>
                      <td style={{ maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {t.subject}
                      </td>
                      <td>
                        <span className={`badge ${ticketPriorityBadge(t.priority)}`}>{t.priority}</span>
                      </td>
                      <td>
                        <span className={`badge ${t.status === 'open' ? 'badge-pending' : 'badge-approved'}`}>{t.status}</span>
                      </td>
                      <td>
                        <Link to={`/admin/tickets/${t.id}`} className="btn btn-secondary btn-xs" style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div className="card">
            <div className="card-title" style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
              <Search size={16} style={{ color: 'var(--br-primary)' }} /> Publisher Lookup
            </div>
          ...
          <div className="card" style={{ padding: 0, flex: 1 }}>
            <div className="card-header" style={{ padding: '16px 20px' }}>
              <div>
                <div className="card-title" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Megaphone size={16} style={{ color: 'var(--br-primary)' }} /> Recent Announcements
                </div>
                <div className="card-subtitle">Broadcast messages sent to publisher portals</div>
              </div>
            </div>
